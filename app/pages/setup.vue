    ...
    <div class="w-full max-w-md">
      <div class="card p-6">
        <div class="text-center mb-4">
          ...
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">创建管理员账号</h1>
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">系统尚未初始化，请注册首个管理员账户</p>
        </div>

        <form @submit.prevent="handleSetup" class="space-y-6">
          <div>
            <label for="setup-email" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">邮箱</label>
            <div class="relative">
          ...
          <div>
            <label for="setup-username" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">用户名</label>
            <div class="relative">
              <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">至少 4 位，不能为纯数字，仅支持英文、数字、下划线</p>
          ...
          <div>
            <label for="setup-password" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">密码</label>
            <div class="relative">
              <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            ...
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">至少 6 位，且不能为纯数字</p>
          </div>
          <div>
            <label for="setup-confirm" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">确认密码</label>
            <div class="relative">
              <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <Icon name="heroicons:lock-closed" class="w-5 h-5 text-gray-400 dark:text-gray-500" />
              </div>
        ...
        <p class="mt-4 text-center text-sm text-gray-500 dark:text-gray-400">
          创建成功后请使用上方账号
          <NuxtLink to="/login" class="text-primary-600 dark:text-primary-400 hover:underline">前往登录</NuxtLink>
        </p>
      </div>
    </div>
  </div>
...
function onUsernameInput() {
  ...
  usernameCheckTimer = setTimeout(async () => {
    ...
    try {
      const res = await $fetch('/api/auth/check-username', { query: { username: raw } })
      if (res.success) {
...
async function handleSetup() {
  ...
  try {
    ...
    if (res.success) {
      toastStore.success('管理员账号已创建，请登录')
